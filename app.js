    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      function buildGridArena(levelIndex) {
        ...
        // ── Bleachers ────────────────────────────────────────────────────────
        {
          ...
          // Build one row strip + spectators along it
          const mkStrip = (cx, cy, cz, w, h, d, specAxis, specLen) => {
            const slab = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), concMat);
            slab.position.set(cx, cy, cz); slab.receiveShadow = true; worldGroup.add(slab);
            const ew = specAxis === 'z' ? 1.5 : w, ed = specAxis === 'x' ? 1.5 : d;
            const cap = new THREE.Mesh(new THREE.BoxGeometry(ew, 2, ed), edgeMat);
            cap.position.set(cx, cy + h / 2 + 1, cz); worldGroup.add(cap);
          ...
          for (let r = 0; r < ROWS; r++) {
            ...
            mkStrip(-(AX + off),  h/2, 0,        RD, h, sideLen, 'z', sideLen); // left
            mkStrip( (AX + off),  h/2, 0,        RD, h, sideLen, 'z', sideLen); // right
            mkStrip(0,  h/2, -(AZ + off),  backLen, h, RD, 'x', backLen);       // back
            mkStrip(0,  h/2,  (AZ + off),  backLen, h, RD, 'x', backLen);       // front

            // Corner fills — connect all 4 sides at each corner
        ...
        if (gameMode === 'online-guest') {
          ...
          if (gameState.bleacherSpectators) {
            ...
            gameState.bleacherSpectators.forEach(s => {
              s.grp.position.y = s.baseY + Math.sin(t * s.speed + s.phase) * s.amp;
              s.aL.rotation.z = -1.1 + Math.sin(t * s.speed + s.phase + 0.5) * 0.4;
              s.aR.rotation.z =  1.1 - Math.sin(t * s.speed + s.phase + 0.5) * 0.4;
            });
          }
          renderer.render(scene, camera);
          return;
